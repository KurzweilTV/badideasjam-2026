...
const MATERIAL_1 = preload("uid://bqhlg48dtyhbi") # the emissive texture
...
@onready var sphere: MeshInstance3D = $blockbench_export/sphere
@onready var sphere_material: StandardMaterial3D = sphere.get_active_material(0).duplicate() as StandardMaterial3D
...
func apply_visual(status: bool) -> void:
	if status:
		...
		sphere_material.emission = indicator_light.light_color
	else:
		anim.play("turn_off")
		await anim.animation_finished
